...
public class AccountServiceReturn {
	...
	public Account[] findByBalance(int balance) {
		 ...
		 */
		Account[] findByBalances = new Account[count];
		/*
		 * C. 만족하는Account객체들 Account배열에담기
		 */
		for (int i = 0, j = 0; i < accounts.length; i++) {
			if (accounts[i].getBalance() >= balance) {
				findByBalances[j] = accounts[i];
				j++;
			}
		}
		return findByBalances;
	}
	...
	public Account[] findByIyul(double iyul) {
		...
		Account[] findByIyuls = new Account[count];
		for (int i = 0, j = 0; i < accounts.length; i++) {
			if (accounts[i].getIyul() >= iyul) {
				findByIyuls[j] = accounts[i];
				j++;
			}
		}
		return findByIyuls;
	...
	public Account[] findByName(String name) {
		 ...
		 * 	- findAccounts=new Account[3];
		 */
		Account[] findByNames = new Account[count];
		/*
		 * C. 만족하는Account객체들 Account배열에담기
		 */
		for (int i = 0, j = 0; i < accounts.length; i++) {
			if (accounts[i].getOwner().equals(name)) {
				findByNames[j] = accounts[i];
				j++;
			}
		}
		return findByNames;
