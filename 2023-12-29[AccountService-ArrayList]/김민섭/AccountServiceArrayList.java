...
public class AccountServiceArrayList {
	...
	*/
	...
	public boolean addAccount(Account account) {
		 ...
		 */
		boolean resultAddAccount = true;
		for (Account accounts : accountList) {
			if (accounts.getNo() == account.getNo()) {
				resultAddAccount = false;
				break;
			}
	...
	public Account ipGum(int no, int m) {
		Account findBN = this.findByNo(no);
		if (findBN != null) {
			findBN.deposit(m);
		}
		return findBN;
		...
		for (int i = 0; i < accountList.size() - 1; i++) {
			for (int j = 0; j < accountList.size() - 1 - i; j++) {
				if (accountList.get(i) != null
						&& accountList.get(j).getBalance() > accountList.get(j + 1).getBalance()) {
	...
	public Account updateAccount(Account updateAccount) {
		for (Account account : accountList) {
			if (account != null && account.getNo() == updateAccount.getNo()) {
	...
	public Account deleteByNo(int no) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			if (accountList != null && accountList.get(i).getNo() == no) {
