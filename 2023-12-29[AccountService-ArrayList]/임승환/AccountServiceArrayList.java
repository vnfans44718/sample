	...
	*/
	...
	public boolean addAccount(Account account) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			Account booleanAccount = (Account)accountList.get(i);
			if (booleanAccount.getNo() == account.getNo()) {
				return false;
			}
		}
	...
	public int totBalance() {
		int totBalance = 0;
		for (int i = 0; i < accountList.size(); i++) {
			Account totBal = accountList.get(i);
			totBalance += totBal.getBalance(); 
		}
		return totBalance;
	}
	...
	public Account findByNo(int no) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			Account noAccount = accountList.get(i);
			if (noAccount.getNo() == no) {
				noAccount.print();
			}
		}
		return findAccount;
	...
	public ArrayList<Account> findByBalance(int balance) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			Account remainBalance = accountList.get(i);
			if (remainBalance.getBalance() >= balance) {
				findAccountList.add(remainBalance);
	...
	/*
	 * 6.6666번계좌 4000원 입금한후 입금계좌 참조변수반환
	 */
	public Account ipGum(int no, int m) {
		Account ipgumAccount = null;
		for (int i = 0; i < accountList.size(); i++) {
			Account currentAccount = accountList.get(i);
			if (currentAccount.getNo() == no) {
				currentAccount.withDraw(m);
				ipgumAccount = currentAccount;
				break;
			}
		}
		return ipgumAccount;
	...
	public ArrayList<Account> findByName(String owner) {
		ArrayList<Account> findAccountList = new ArrayList<Account>();
		for (int i = 0; i < accountList.size(); i++) {
			Account ownerAccount = accountList.get(i);
			if (ownerAccount.getOwner().equals(owner)) {
				findAccountList.add(ownerAccount);
			}
		}
		return findAccountList;
	}
	...
	public ArrayList<Account> findByVip(int balance) {
		ArrayList<Account> findAccountList = new ArrayList<Account>();
		for (int i = 0; i < accountList.size(); i++) {
			Account vipBalance = accountList.get(i);
			if (vipBalance.getBalance() >= balance) {
				findAccountList.add(vipBalance);
	...
	public Account updateAccount(Account updateAccount) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			...
			if (account2.getNo() == updateAccount.getNo()) {
				accountList.set(i, account2);
			}
		}
		return null;
	}
	...
	public Account deleteByNo(int no) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			Account deleteAcc = accountList.get(i);
			if (deleteAcc.getNo() == no) {
