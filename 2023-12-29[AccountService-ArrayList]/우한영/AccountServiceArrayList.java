...
public class AccountServiceArrayList {
	...
	*/
	...
	public ArrayList<Account> findByVip(int balance) {
		ArrayList<Account> findAccountList = new ArrayList<Account>();
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount7 = accountList.get(i);
			if (tempAccount7.getBalance() >=70000) {
				findAccountList.add(accountList.get(i));
			}
	...
	public void sortByBalanceAsc() {
		for (int i = 0; i < accountList.size()-1; i++) {
			for (int j = 0; j < accountList.size()-1; j++) {
				Account tempAccount8 = accountList.get(j);
				Account tempAccount9 = accountList.get(j+1);
				if(tempAccount8.getBalance() > tempAccount9.getBalance()) {
					accountList.set(j+1, accountList.set(j, tempAccount9));
	...
	public Account updateAccount(Account updateAccount) {
		Account tempAccount11 =null;
		for (int i = 0; i < accountList.size(); i++) {
			if(accountList.get(i).getNo() == (updateAccount.getNo())) {
				accountList.set(i,updateAccount);
				tempAccount11 = accountList.get(i);
			}
		}
		return tempAccount11;
	}
