...
public class AccountServiceArrayList {
	...
	*/
	...
	public  Account ipGum(int no, int m) {
		Account ipGum = new Account();
		for (int i = 0; i < accountList.size(); i++) {
			if(accountList.get(i).getNo()==no) {
				accountList.get(i).deposit(m);
				ipGum = accountList.get(i);
				break;
			}
		}
		return ipGum;
	...
	/*
	 * 9.1111번계좌 50000원 출금한후 출금계좌 참조변수반환
	 */
	public Account chulGum(int no, int m) {
		Account chulGum = new Account();
		for (int i = 0; i < accountList.size(); i++) {
			if(accountList.get(i).getNo()==no) {
				accountList.get(i).withDraw(m);
				chulGum = accountList.get(i);
				break;
			}
		}
		return chulGum;
	...
	public Account updateAccount(Account updateAccount) {
		for (int i = 0; i < accountList.size(); i++) {
			if(accountList.get(i).getNo()==updateAccount.getNo()) {
				accountList.get(i).setOwner(updateAccount.getOwner());
				accountList.get(i).setBalance(updateAccount.getBalance());
				accountList.get(i).setIyul(updateAccount.getIyul());
				break;
			}
		}
