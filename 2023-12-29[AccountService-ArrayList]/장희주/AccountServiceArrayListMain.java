...
public class AccountServiceArrayListMain {
	...
	public static void main(String[] args) {
		...
		ArrayList<Account> findAccounts1 = accountServiceArrayList.findByBalance(50000);
		System.out.println(" 반환계좌수:" + findAccounts1.size());
		for (Account account : findAccounts1) {
			account.print();
		}

		System.out.println("7.AccountServiceArrayList야은행계좌들 중에 계좌주이름이 AIM인 계좌들반환해줘");
		ArrayList<Account>findAccounts2 = accountServiceArrayList.findByName("AIM");
		for (Account account : findAccounts2) {
			account.print();
		}
		/*
		 * 8.6666번계좌 4000원 입금해줘1
		 */
		...
		ArrayList<Account>findAccounts3 = accountServiceArrayList.findByBalance(70000);
