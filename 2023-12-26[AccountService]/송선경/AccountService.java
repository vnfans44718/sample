...
public class AccountService {
	...
	*/
	...
	public void totAccountNumberPrint() {
		System.out.println("총계좌 수: "+ accounts.length);
	}
	/*
	 * 2.은행계좌들 전체출력메쏘드 정의
	 */
	...
	public void totBalancePrint() {	
		for (int i = 0; i < accounts.length; i++) {
			System.out.println("총 잔고 : "+accounts[i].getBalance());	
		}
	}
	/*
	...
	*/ 
	...
	/*
	8.계좌번호,입금할돈 인자로 받아서 입금 (1.계좌번호로 계좌찾기 /2.입금)
	 */ 
	public void ipGum(int no,int m) {
		for (int i = 0; i < accounts.length; i++) {
			if(accounts[i].getNo()==no){
				accounts[i].setBalance(accounts[i].getBalance()+m);
				 System.out.println("입금 후 잔고: " +(accounts[i].getBalance()+m));
			}
		}
	}
	/*
	...
	public void chulGum(int no,int m) {
		for (int i = 0; i < accounts.length; i++) {
			if(accounts[i].getNo()==no){
				accounts[i].setBalance(accounts[i].getBalance()-m);
				System.out.println("출금 후 잔고: " +(accounts[i].getBalance()-m));
			}
		}
	}
	/*
