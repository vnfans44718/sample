package com.itwill05.serivce.account;

public class AccountServiceMain {
	public static void main(String[] args) {
		
		AccountService accountService = new AccountService();
		
		System.out.println("0.AccountService객체야 Account객체줄게 추가해줘(배열사이즈증가) [OPTION]");
		Account temp1 = new Account(1234, "Ariel", 57000, 2.2);
		accountService.addAccount(temp1);
		accountService.print();
		
		System.out.println("0.AccountService객체야 번호,이름,잔고,이율줄게 계좌객체 추가해줘(배열사이즈증가) [OPTION]");
		accountService.addAccount(new Account(5678, "Rapunzel", 23000, 9.9));
		accountService.print();
		
		System.out.println("1.AccountService객체야 은행계좌들 총계좌수 출력해줘");
		accountService.totAccountNumberPrint();
		
		System.out.println("2.AccountService객체야 은행계좌들 전체 출력해줘");
		accountService.print(); 
		
		System.out.println("3.AccountService객체야 은행계좌들 총잔고 출력해줘");
		accountService.totBalancePrint();
		
		System.out.println("4.AccountService객체야 계좌번호인자로줄게 계좌한개 출력해줘");
		accountService.findByNoPrint(9999);
		
		System.out.println("5.AccountService객체야 계좌잔고인자로줄게 잔고이상인계좌들 출력해줘");
		accountService.findByBalancePrint(89000);
		
		System.out.println("6.AccountService객체야 계좌이율인자로줄게 이율이상인계좌들 출력해줘");
		accountService.findByIyulPrint(4.7);
		
		System.out.println("7.AccountService객체야 계좌주이름인자로줄게 계좌주이름과일치하는계좌들 출력해줘");
		accountService.findByNamePrint("XIM");
		
		System.out.println("8.AccountService객체야 계좌번호,입금할돈 인자로줄게 입금해줘");
		accountService.ipGum(1111, 5000);
		accountService.print();
		
		System.out.println("9.AccountService객체야 계좌번호,출금할돈 인자로줄게 출금해줘");
		accountService.chulGum(3333, 5000);
		accountService.print();
		
		System.out.println("10.AccountService객체야 계좌를 잔고순으로 오름차순정렬해줘");
		accountService.sortByBalanceAscending();
		accountService.print();
		
		System.out.println("11.AccountService객체야 계좌를 잔고순으로 내림차순정렬해줘");
		accountService.sortByBalanceDescending();
		accountService.print();
		
		accountService.sort(AccountService.SORT_BY_NO, AccountService.SORT_ASC);
		accountService.print();
		
		System.out.println("12.AccountService객체야 계좌객체줄게 이름,잔고,이율 수정해줘(update) [OPTION]");
		Account temp2 = new Account(2222, "Orora", 26000, 8.8);
		accountService.updateAccount(temp2);
		accountService.print();
		
		System.out.println("13.AccountService객체야 번호,이름,잔고,이율줄게 계좌객체 수정해줘(update) [OPTION]");
		accountService.updateAccount(4444, "Bell", 99000, 5.5);
		accountService.print();
		
	}
}
