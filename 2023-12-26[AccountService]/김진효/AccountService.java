...
public class AccountService {
	...
	*/
	public void addAccount(int no, String owner, int balance, double iyul) {
		...
		tempAccount[tempAccount.length - 1] = new Account(no, owner, balance, iyul);
		accounts = tempAccount;
	}

	// 1.은행계좌들 총계좌수출력메쏘드정의
	public void totAccountNumberPrint() {
		System.out.println("총계좌수 : " + accounts.length + "개");
	}
	...
	public void totBalancePrint() {
		for (int i = 0; i < accounts.length; i++) {
			System.out.println(accounts[i].getOwner() + " 님의 총잔고: " + accounts[i].getBalance() + "원");
		}
	}
	...
	public void findByNamePrint(String name) {
		for (int i = 0; i < accounts.length; i++) {
			String tempName = accounts[i].getOwner();
