...
public class CarServiceMain {
	public static void main(String[] args) {
		...
		carService.print();
		boolean isSuccess=
		carService.ipCha(new Car("2344",10));
		if(!isSuccess) {
		...
		int x=carService.getTotalParkingNumber();
		System.out.println(x);
		
		System.out.println("3.CarService객체야 주차가능주차구획수반환해줘");
		int y=carService.getAvailableParkingNumber();
		...
		Car[] findIyCha = carService.findByInTime(8);
		for (Car car : findIyCha) {
			car.print();
		}
		System.out.println("6.CarService객체야 차량번호(1212번) 출차시간(12시)인자로줄께 출차시켜주고 출차차량참조변수반환해줘");
		Car findCulCha=carService.chulCha("1212", 12);
		findCulCha.print();
	}
