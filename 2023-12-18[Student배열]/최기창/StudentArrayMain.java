...
public class StudentArrayMain {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < students.length - 1; i++) {
			for (int j = 0; j < students.length - 1 - i; j++) {
				if (students[j].getTot() < students[j + 1].getTot()) {
					Student tottemp = students[j];
					students[j] = students[j + 1];
					students[j + 1] = tottemp;
		 ...
		 * 7. 학생학점순으로 오름차순정렬
		 */
		System.out.printf("6. 학생학점순으로 오름차순정렬");
		for (int i = 0; i < students.length; i++) {
			for (int j = 0; j < students.length; j++) {
				if (students[j].getGrade() < students[j].getGrade()) {
					Student gradetemp = students[j];
					students[j] = students[j + 1];
					students[j + 1] = gradetemp;
				}
			}
